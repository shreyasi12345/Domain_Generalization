...
if __name__ == "__main__":
    ...
    parser.set_defaults(
        ...
        num_workers=0,
        t=1.0,
        loss_function='EntropyMaximization'
        )
    ...
    if(args.loss_function=='EntropyMximization'):
        print("t=",args.t)
        print("using entropy maximization loss")
        logging.info("using entropymaximization loss")
        enm_loss = losses.EntropyMaximization(t=args.t)
    elif(args.loss_function=='MMD'):
        print("using mmd loss")
        logging.info("using mmd loss")
        enm_loss= losses.MMDLoss()
    elif(args.loss_function=='DomainAdversarialLoss'):
        print("using DomainAdversarialLoss ")
        logging.info("using DomainAdversarialLoss")
        enm_loss= losses.DomainAdversarialLoss()
    ...
    for epoch in range(args.num_epochs):
        ...
        torch.save(net.state_dict(), os.path.join(model_dir, '{:05d}.pth'.format(epoch+1)))
        print('model saved.')
